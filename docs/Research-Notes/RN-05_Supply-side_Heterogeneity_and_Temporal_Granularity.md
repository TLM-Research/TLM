...
**Value is unevenly distributed within a block, not only across slots.** Franco and Rogozinski observe that Ethereum block value concentrates at the top: the first few transactions - arbitrage legs, liquidations, oracle-sensitive trades - capture a disproportionate share of what is extracted over a slot, while the remainder settles into a quieter, lower-value regime. They read this as an argument for running more auctions per slot. Read against this note it is evidence of something else: intra-slot *position* carries heterogeneous economic value, which is what execution priority (sec. 3) names and what a uniform slot boundary compresses away.
...
**A committee or pipelined variant sits between A and C, and is not dismissed here.** The three options above are stated as a clean frontier, but intermediate designs exist: a rotating attester sub-committee witnessing arrivals without full validator-set agreement, or a pipelined scheme in which commitment and verification overlap successive quanta. These weaken Option C's latency floor without reaching Option B's pure post-hoc posture, and one of them may well be where an arrival witness (sec. 4.3) comes from. The variant is not hypothetical: Franco and Rogozinski's **mini-blocks** design runs per-round agreement inside a distributed-validator cluster rather than across the validator set, reaching a 2-second sub-slot cadence that way. This note does not evaluate such designs - doing so requires the mechanism analysis it excludes - but the A/B/C partition should be read as three reference points on a frontier, not an exhaustive enumeration.
...
- Franco, M. & Rogozinski, G. *Mini-Blocks: SSV-Backed Sub-Slot Auctions for Ethereum PBS.* Ethereum Research, May 2026. https://ethresear.ch/t/mini-blocks-ssv-backed-sub-slot-auctions-for-ethereum-pbs/24898 (Top-of-block value concentration; DVT-cluster per-round agreement.)

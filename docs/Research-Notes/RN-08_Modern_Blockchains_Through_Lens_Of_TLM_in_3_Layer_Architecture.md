...
These are **commodity** for our purpose: every chain has them, and differences here are performance, not kind. One caution the conclusion depends on: "commodity" means every chain *has* these parts, not that the parts are interchangeable across chains. Consensus, state, ordering, and fees carry different safety, data-availability, validator-duty, and governance interfaces from one chain to the next, so the differences are not only performance - they are interface differences. This matters in sec. 9: combining the four chains' differentiated features is a *redesign under a common interface*, not the assembly of drop-in modules. What separates the four candidates is the **differentiated feature** each adds on top:
...
**Why these four, and what they cannot show.** The four are chosen as *representative* poles - Ethereum for neutral settlement, Aptos and Monad for two parallel-execution designs, HyperCore for multi-domain execution under one consensus - not as a census. That selection is deliberate, and it also shapes the conclusion (sec. 9), so it should be read with the cases it omits: rollups and shared sequencers, Polkadot coretime, Cosmos app-chains, Solana, and Sui - several of which could challenge the decoupling direction (a rollup already separates execution while inheriting settlement; shared security without shared state is a different point on the triangle of sec. 9). A capacity-frontier measurement across a broader sample is the simulation program's task (sec. 10), not this survey's; what the survey owes, and states here, is that its sample is representative and its conclusion is therefore a motivated hypothesis, not a proof.
...
Second, the right yardstick is not TPS but **Temporal Economic Throughput (TET)** - how much economically useful demand is satisfied over time, weighting deadline success, delay, predictability, capacity utilization, and value preserved by correct timing. Two hosts with equal TPS can have very different TET; one that protects time-critical demand while deferring flexible work scores higher. TET is best read as a **vector of metrics**, not a single value-weighted scalar: utilization, deadline and cadence success, delay and jitter, non-participant service, extraction, validator burden, and concentration. A single aggregate can rise while ordinary demand is starved or extraction grows, so the components must be reported alongside it - value-weighting must not conceal distribution.
...
The direction this survey motivates is to **decouple the execution engine from the control engine**: share one credibly-neutral consensus and settlement base, run a temporal control engine above it (RN-07), and let execution be differentiated - multiple engines, runtimes, ordering rules, and fee policies - without each one duplicating consensus. This is TLM's core move. It is stated here as a **hypothesis the comparison motivates, not a result it proves**: the lens presupposes separable control and execution, and the four systems were chosen because they occupy complementary roles, so the survey can motivate the decoupling but cannot by itself establish it over competing decompositions - better rollups, shared security without shared state, uniform execution with richer scheduling, or sovereign chains. Testing it against those alternatives on a common workload is the next step (sec. 10).

The hypothesis rests on one **precondition** that the survey does not settle and that is not a downstream detail: differentiated execution over *one shared, atomically composable state* requires a root serialization rule for cross-domain state access, plus a validation path for domains that not every validator executes (committee, fraud proof, or validity proof). Where those hold, execution can differ under shared state; where they do not, the decoupling gives back the composability it was meant to keep, and 'inherited' security no longer approximates the host. This is the sovereignty / atomic-state / host-validation triangle of RN-09, and it conditions the conclusion. How the architecture should be built - and whether that triangle can be met cheaply - is what RN-09 and the mechanism notes take up.

The downstream question - *which existing chain is best positioned to serve as that shared base* - follows from the same analysis, and its answer is a **direction, not a ranking**: **Ethereum** contributes neutral settlement; **Monad**, EVM-compatible performance; **Aptos**, a programmable parallel data plane to prototype a control engine over; and **HyperCore**, the instance showing differentiated execution under one consensus. But "combine these" is not module assembly. Because the four carry different safety, state, validation, and governance interfaces (sec. 4), joining their strengths is a **redesign under a common interface**, not plugging in drop-in parts - what carries over is the *lesson* each teaches, and the combined design must re-derive the interfaces rather than inherit them. No single chain is complete today, and the strongest *design* would seek neutral settlement, a high-performance parallel data plane, multi-domain execution under one consensus, and a temporal control engine whose policies respect the RN-07 reducibility invariant - built as one system, not bolted together.

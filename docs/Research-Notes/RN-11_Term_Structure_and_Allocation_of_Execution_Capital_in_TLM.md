...
## 0.5 Numeraire, the fee, and the storability of blockspace

The conceptual ground for Part I -- why a non-storable execution service can carry a bond-like term structure at all -- is developed in **RN-10 sec. 3** and assumed here rather than repeated. In brief: blockspace is often read as a perishable commodity (electricity), which would give it only a scarcity-driven commodity-forward curve. RN-10 sec. 3 argues this stops one level too early. The network is durable capital producing a non-storable per-slot service flow (a stock-flow correction); and, the decisive point, the fee is quoted in the **native coin**, which is storable and yield-bearing and is itself a claim on the capital that produces blockspace. The value dimension of the market therefore lives in a storable, endogenous numeraire, and blockspace sits on a spectrum between a pure commodity and money. Money is the frictionless limit of a temporal market, not a different kind of object; **temporal liquidity is the dial** that moves blockspace toward it, by supplying the demand-side substitution that storage supplies for a commodity. Part I prices execution on that reading.

**Execution capital, the object in the title.** It is defined canonically in **RN-10 sec. 3** and used here in that sense: *blockspace viewed as capital* -- the network's durable, capacity-producing capability. Its per-slot service flow is **blockspace** (execution capacity), the thing the term structure prices (Part I) and the allocation allocates (Part II); its **capital value**, the principal, is the present value of the fees that flow will earn (sec. 2.3). The three stay distinct here as in RN-10: the capital is the stock, blockspace is its flow, the capital value is what the stock is worth. So "the allocation of execution capital" (Part II) means allocating that flow across time -- deploying the capital's output -- not dividing the stock, as one allocates human capital by deploying labor.

Three objects from RN-10 secs. 3.1-3.2 are used throughout Part I and are the minimum needed here:

- **the discount factor B(0,t)** -- the coin's *own-rate*, the yield the aggregate fee stream throws off on the capital that produces it (B = R/P + g), not the staking yield (which adds issuance). Over a single slot B is approximately 1, so discounting matters only across the multi-slot horizon.
- **the fee as a rental rate, not an interest rate** -- the price of one period of deployed capacity, in coin; B is the interest rate, the fee is not, and compounding lives in B.
- **the fee as an abstraction** -- the unified, protocol-visible price a TLM-enabled protocol would expose, into which today's base fee, priority tip, and MEV are folded (RN-10 sec. 3.2); where Part I says "fee rate" it means this cleaned price.

On this basis Part I keeps three price objects distinct: the money discount B(0,t), the spot service price S_g(t), and the forward service price F_g(0,t) (sec. 1).
...
|---|---|---|
| Overnight / spot rate | Spot inclusion: the cleaned unit fee now (sec. 0.5) | Exists (as base fee + tip) |
...
The curve is bootstrapped from traded instruments, all defined on the cleaned fee (sec. 0.5).

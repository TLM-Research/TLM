...
These are unified by a formal statement of the problem, given in **RN-11**: the protocol allocates capacity across quanta so as to maximize value-weighted execution subject to capacity, neutrality, reducibility, and incentive compatibility, and **the block-fee-rate curve is the vector of shadow prices on per-quantum capacity in that problem**. That statement turns several of the arguments below, including fee-as-filter, into consequences of the dual. This note stays at the level of economic structure and does not develop it.
...
## 3. Two views of blockspace, and why TLM takes the second

Blockspace can be viewed two ways. Both are valid; they answer different questions, and the difference is the hinge of this note.

**View 1 - blockspace as a perishable resource (the conventional view).** Take a single slot in isolation. Its capacity is perishable: whatever is not used in slot *t* is gone and cannot be recovered - like an unsold seat on a departed flight, or an idle CPU cycle. Time is an exogenous index, a clock that labels slots, and the only economic question is *who gets this block*. This is the view behind a scalar spot fee such as EIP-1559: a spot market for a perishable good, priced one block at a time.

**View 2 - blockspace as reusable capital (the TLM view).** Now look at the network as a going concern. Blockchain time is one-directional and unbounded, and the network re-produces execution capacity at slot (quantum) frequency, slot after slot, indefinitely. Seen this way, the capacity-producing infrastructure is durable **capital** that repeatedly creates value - through transactions among decentralized parties - and it is **not** perishable: capacity undeployed in a slot is reserve capital that was not lent out, and the capital persists to produce again next slot. What is scarce and worth pricing is not one perishable block but the *allocation of this ongoing capacity across time*.

| | View 1: perishable resource (conventional) | View 2: reusable capital (TLM) |
|---|---|---|
| Unit of analysis | a single slot's blockspace | the network as durable, capacity-producing capital |
| Unused capacity | lost - a passed slot is gone | undeployed reserve - the capital persists and produces again |
| Role of time | exogenous index ("now vs not-now") | endogenous, protocol-visible, first-order variable |
| Resource dimension | one-dimensional (capacity at a point) | two-dimensional (capacity by time) |
| Market form | spot market for the next block | capacity allocated across time - spot plus forward / leases, service classes |
| Question answered | who gets this block? | how to finance value creation over time, across projects? |
| Canonical instance | EIP-1559 scalar fee | TLM |

Both views describe the same object correctly at different scopes: View 1 is true of a slot taken alone; View 2 is true of the network as a capital that keeps producing. **TLM takes View 2**, and that choice is the foundation of the program: it is only under View 2 that **temporal structure becomes an endogenous, protocol-visible, first-order variable** rather than an exogenous index. Under View 1, time merely labels *when* things happen. Under View 2, *when, in what order, over what horizon, and how predictably* value is created are decision variables that the protocol and its participants act on - so the resource is two-dimensional (capacity by time), and the second dimension carries information a market can coordinate. What the protocol allocates is then not a scalar "blockspace" but positions on a **capacity-by-time grid**: capacity used in a slot is *deployed*, capacity unused is *undeployed reserve* (sec. 4).
...
The conventional fee view - "urgent demand pays more" - is a single, volatile *spot* price. In finance that is only the shortest point on a whole **term structure**. The time value of money is not one number but a curve of rates across horizons, and blockspace has a direct and useful analog.
...
Two continuations are immediate. The **formal statement** in RN-11 identifies the block-fee-rate curve as the dual of the allocation problem, and turning that into results is the program's central mathematical task. **Virtual chains** (RN-09) are the architectural counterpart of the demand-side argument made here: they are how "financing a diverse ecosystem of projects" becomes something a host chain can actually do.
...
6. **Is the formal statement (RN-11) the right abstraction?** It claims the parts of this note are facets of a single allocation problem whose dual is the block-fee-rate curve. Is that formulation correct and well-posed; is the duality argument sound; and is this the right central problem for the program, or is there a better one?
